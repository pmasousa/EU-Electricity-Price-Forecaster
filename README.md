<div align="center">
  ...
  <p><i>A probabilistic day-ahead electricity price forecasting stack — Portugal and Switzerland shipped end-to-end, Spain in progress.</i></p>
  <p>
...
**Status:** PT and CH are trained, benchmarked, and served end-to-end. ES is registered in the config with data downloaded and its benchmark run in progress — run the pipeline for a country before claiming it.
...
**Notes:** Classical models take the point forecast in both markets (tie on CH; Linear Regression ahead on PT — the harder market at rMAE 0.90 vs 0.78). The TFT's edge is market-dependent: best pinball on CH, behind quantile-LightGBM on PT. q90 upside breaches exceed the 10% calibration target in both markets (PT > 50%, against a holdout where prices nearly tripled their 3-year mean) — the driver for the v1.1 roadmap (neighboring-zone prices, day-ahead load forecasts). Served models are the benchmarked TFTs as-is (`build_serving.py`, no post-selection retrain). The pre-2026-08-25 metrics files used incompatible protocols (per-model splits, single-hour scoring, realized-load leakage) and were removed.
