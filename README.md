<div align="center">
  ...
  <p><i>A probabilistic day-ahead electricity price forecasting stack — Switzerland shipped end-to-end, Portugal and Spain config-ready.</i></p>
  <p>
...
**Status:** CH is trained, benchmarked, and served end-to-end. PT and ES are registered in the config (data + training + serving are one command each) but ship **no artifacts** yet — run the pipeline for a country before claiming it.
...
## 📊 Benchmark — CH day-ahead

Generated by `src/evaluation/backtest.py` (current tables in `reports/benchmark_CH_*.txt`; every run archived with its model and data snapshot under `reports/runs/`).

**Protocol:** 26,016 hourly rows (2023-09 → 2026-08, Energy-Charts + Open-Meteo) · train / 3-week early-stop val / 8-week holdout · day-ahead walk-forward (24h horizon, daily stride, frozen models — no retraining at origin) · MAE/RMSE in EUR/MWh across **all 24 forecast hours** · rMAE = MAE relative to yesterday's-curve persistence.

**Honest covariates:** calendar + weather at forecast time (realized weather as a day-ahead forecast proxy) + load lagged 24h/168h. Realized load at forecast time is excluded — it is not known at day-ahead gate closure. All scalers fit on the train split only.
...
| TFT (minmax, longer early-stop leash) | 23.00 | 31.30 | 1.08 |
| TFT (asinh, 5q) | 28.19 | 35.04 | 1.32 |

**Probabilistic scoreboard** — pinball loss at q10/50/90 (EUR/MWh); a calibrated model breaches below q10 / above q90 about 10% / 10% of hours:

| Model | pinball mean | below q10 | above q90 |
|---|---|---|---|
| **TFT (minmax)** | **6.49** | 6.9% | 31.7% |
| LightGBM (quantile) | 6.96 | 17.4% | 36.6% |

**Honest read:** with this feature set the classical models beat yesterday's curve by ~22% on the point forecast (LightGBM and Linear Regression are a statistical tie), and **the TFT wins the distribution forecast** — better pinball and better-calibrated tails. Both model families still underestimate upside risk (~32–37% of hours breach q90 vs the 10% target) on a holdout that sits in a rising-price regime — the documented motivation for the v1.1 feature roadmap (neighboring-zone prices, day-ahead load forecasts). The served model is the benchmarked TFT as-is (`build_serving.py` copies it, no post-selection retrain). These tables replace the pre-2026-08-25 `metrics.txt` files, which mixed incompatible protocols: different splits per model, scoring on a single horizon hour, and realized-load leakage.
